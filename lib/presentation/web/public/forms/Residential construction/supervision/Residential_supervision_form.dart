...
import 'supervision_notifier.dart';

class SupervisionForm extends ConsumerWidget {
  const SupervisionForm({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final formKey = GlobalKey<FormState>();
    final state = ref.watch(supervisionProvider);
    final notifier = ref.read(supervisionProvider.notifier);

    ref.listen(supervisionProvider, (prev, next) {
      if (next.isSuccess) {
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    Expanded(
                      child: Text(
                        AppTranslations.get('supervision_form_title', lang),
                        style: TextStyle(
                ...
                FormSection(
                  ...
                  children: [
                    Row(children: [
                      ...
                      const SizedBox(width: 24),
                      ServiceTypeButton(
                        label: AppTranslations.get('project_supervision', lang),
                        isSelected: true,
                      ),
                    ]),
                    const SizedBox(height: 32),
                    Text(AppTranslations.get('existing_proj_dev', lang),
                        style: const TextStyle(fontWeight: FontWeight.bold)),
                    const SizedBox(height: 16),
                    FormRow(children: [
                      RadioOption(label: AppTranslations.get('yes', lang), isSelected: state.isExistingProject,
                          onTap: () => notifier.setIsExistingProject(true)),
                      const SizedBox(width: 24),
                      RadioOption(label: AppTranslations.get('no', lang), isSelected: !state.isExistingProject,
                          onTap: () => notifier.setIsExistingProject(false)),
                    ]),
