...
final constructionProvider =
    NotifierProvider<ConstructionNotifier, ConstructionState>(
  ConstructionNotifier.new,
);

// ─────────────────────────────────────────────
// Notifier
// ─────────────────────────────────────────────
class ConstructionNotifier extends Notifier<ConstructionState> {
  @override
  ConstructionState build() => const ConstructionState();
  ...
  // ── تحديث الاختيارات ─────────────────────
  void setConstructionServiceType(String v) => state = state.copyWith(constructionServiceType: v);
  ...
  Future<void> submitForm() async {
    ...
    try {
      ...
      await supabaseService.client.from('project_service_forms').insert({
        ...
        'service_id': 2,
        'form_type': 'Residential_construction',
        'form_data': {
          ...
          'metadata': {
            'construction_service_type': state.constructionServiceType,
  ...
  void reset() => state = const ConstructionState();
}
